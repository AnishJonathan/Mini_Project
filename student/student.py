...
def student_quiz():
    ...
    if quiz_det:
        ...
        dt1 = datetime.datetime.strptime(datetime.datetime.now(pytz.timezone("Asia/Kolkata")).strftime('%Y-%m-%d'), '%Y-%m-%d').date()
        print("dt1",dt1)
            ...
            if rec is None:
                if session.get('submitted_ques') is None or session.get('q_nos') is None:
                    ...
                    if user_attempt_ques is None:
                        ...
                        ques_type = [int(x) for x in user_attempt_ques['ques_type'].split(',') if x!=""]
                        one_line_ans_st = user_attempt_ques['one_line_ans']
                        mcq_st = user_attempt_ques['selected_opt']
                        desc_ans_name_st = user_attempt_ques['desc_ans_name']
                        print(user_attempt_ques,"user_attempt_ques")
                        ...
                        for i in range(len(ques_type)):
                            if ques_type[i] == 0:
                                ...
                                answers.append(desc_ans_name_st[desc_ans_name_ct])
                                desc_ans_name_ct += 1
...
def question_submit_mode():
    if request.method == "POST" or request.method == "GET" :
        ...
        if request.method == "POST":
            ...
            if ans_type==0:
                ...
                if len(request.files) != 0:
                    desc_ans = request.files["desc_ans"]
                    desc_ans_name =  ques_no + "N-N" + desc_ans.filename
                    directory = os.path.join(app_root,"student/descriptive_ans_uploads",session['dept'],str(session['sem']),str(session['quiz_id']),str(session['svv']),str(session['current_ques']))
                    desc_ans_file = os.path.join(directory,desc_ans_name)
                    session["desc_ans_file"] = {session['current_ques']:desc_ans_file}
                    Path(directory).mkdir(parents=True, exist_ok=True)

                    for f in os.listdir(directory):
                        os.remove(os.path.join(directory, f))
                    
                    desc_ans.save(os.path.join(directory,secure_filename(desc_ans_name)))
                else:
                    desc_ans_name = ques_no + 'N-N' + request.form["desc_name"] 
        ...
        if user_attempt_ques is not None:
            ...
            user_submit_ques_splt = [int(x) for x in user_submit_ques['ques_id'].split(',') if x!=""]
            
            
            ques_type = [int(x) for x in user_submit_ques['ques_type'].split(',') if x!=""]
            one_line_ans_st = user_submit_ques['one_line_ans']
            mcq_st = user_submit_ques['selected_opt']
            desc_ans_name_st = user_submit_ques['desc_ans_name']
            ...
            if user_submit_ques['desc_ans_name']:
                desc_ans_name_st = user_submit_ques["desc_ans_name"].split(',')
        ...
        if mode=="next" or mode=="prev":
            ...
            if int(ques_id) not in session['not_submitted_ques_id'] and int(ques_id) not in session['ques_submitted']:
                ...
                if ans_type==0:
                    ...
                    session['to_be_submitted_answer'].append(desc_ans_name)
                prev_attempt = 0
                ...
                if way==0:
                    ...
                    if ans_type== 0:
                        ...
                        to_be_submitted_answer[not_submitted_ques_id_index] = desc_ans_name
        ...
        ################ Removing question ID's from the session when it is submitted
            if ques_id in session['not_submitted_ques_id'] and ques_id in session['ques_submitted']:
                ...
                session['to_be_submitted_answer'] = to_be_submitted_answer
        ##############
        if session['current_ques'] in user_submit_ques_splt and session['current_ques'] in session['ques_submitted']:    
...
@student.route('/quiz_submitted/',methods=['GET'])
@login_required
def show_submit_page():
    return render_template('quiz_submit.html',msg="Quiz has been ended by Faculty, Your responses has been recorded!",score="",total="")
...
def question_submit():
    if session.get('submitted_ques') is not None or session.get('q_nos') is not None:
        ...
        if user_attempt_ques == 0:
            ...
            user_submit_ques_splt = [int(x) for x in record['ques_id'].split(',') if x!=""]
            ...
            if int(ques_id) not in user_submit_ques_splt:
                ...
                if ans_type==0:
                    ...
                    if record['desc_ans_name'] is not None:
                        updt_ans = record['desc_ans_name']+'$,'+desc_ans_name
                    else:
                        updt_ans = desc_ans_name
                    updt_ans_file = record['desc_ans_file']

                    cur.execute("UPDATE quiz_responses SET desc_ans_name = %s, desc_ans_file=%s, ques_type = %s , ques_id = %s , time_per_ques = %s WHERE quiz_id = %s AND user_inserted=%s" ,(updt_ans,updt_ans_file,updt_ques_type,updt_ques_id,end_time,session['quiz_id'],session['svv']))
            
            elif int(ques_id) in user_submit_ques_splt: # finding index and removing item from the answer submitted list
                ques_type = [int(x) for x in record['ques_type'].split(',') if x!=""]
                one_line_ans_st = record['one_line_ans']
                end_time = record['time_per_ques'].split(',')
                mcq_st = record['selected_opt']
                desc_ans_name_st = record['desc_ans_name']

                if record['one_line_ans']:
                    one_line_ans_st = record['one_line_ans'].split(',')
                if record['selected_opt']:
                    mcq_st = record['selected_opt'].split(',')
                if record['desc_ans_name']:
                    desc_ans_name_st = record['desc_ans_name'].split(',')
                    ...
                    if i==user_submit_ques_splt_index:
                        if ques_type[i] == 0:
                           mcq_st.pop(mcq_ct)
                            ...
                            desc_ans_name_st.pop(desc_ans_name_ct)
                    if ques_type[i] == 0:
                        mcq_ct += 1
                    elif ques_type[i] == 1:
                        one_line_ct += 1
                ...
                if desc_ans_name_st:
                    updt_desc_ans_name =','.join(desc_ans_name_st)
                ...
                if ans_type==0:
                    ...
                    if updt_desc_ans_name == "":
                        updt_desc_ans_name += updt_desc_ans_name
                    else:
                        updt_desc_ans_name += ','+ updt_desc_ans_name
                ...
                if ans_type==0:
                    ...
                    cur.execute("UPDATE quiz_responses SET desc_ans_name = %s , ques_type = %s , ques_id = %s , time_per_ques = %s WHERE quiz_id = %s AND user_inserted=%s" ,(updt_desc_ans_name,updt_ques_type,updt_ques_id,end_time,session['quiz_id'],session['svv']))
    ...
    if mode=="Faculty":
        ...
        if quiz_response:
            ...
            if quiz_response['desc_ans_name'] is not None:
                res_desc_ans_name = quiz_response['desc_ans_name'].split("$,")
            else:
                res_desc_ans_name = []
            ...
            for i in range(len(questions)):
                ...
                if len(ques_type)>i:
                    if ques_type[i] == '0':
                        ...
                        answers.append(res_desc_ans_name[desc_ans_name_ct])
                        desc_ans_name_ct += 1
                else:
                    # #
...
@student.route('/change_electives/',methods=['POST', 'GET'])
@login_required
def electives_change():
    print("change elective")
    print(request.form)
    res = {}
    ...
    if stud_id != None and selected_elect != None:
        ...
        if selected_electives['electives'] is not None:
            ...
            sel_elect = []
        print(sel_elect)
        if selected_elect not in sel_elect:
            sel_elect.append(selected_elect)
            join_electives = ','.join(map(str, sel_elect))
            print(join_electives)
            cursor.execute("UPDATE student SET electives = %s WHERE S_id = %s AND dept = %s",(join_electives,stud_id,session['dept']))
            mysql.connection.commit()
            res['updated'] = 1
            msg = {}
            msg['title'] = "Successfully Updated Elelctive!"
            msg['there'] = 1
            msg['mode'] = 1
            session['message'] = msg
        else:
...
@student.route('/check_quiz',methods=['POST', 'GET'])
@login_required
def quiz_show():
    if session['mode'] == "Student" and (session['sem'] == 8 or session['sem'] == 7):
        ...
        if cur.rowcount == 0:
            ...
            return render_template('check_quiz.html',message="",q_details=records)
    else:
        return redirect(url_for('dashboard'))
