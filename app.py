...
app.config['SEND_FILE_MAX_AGE_DEFAULT'] = 0 #Remove during production
...
def auth_decode(url,pwd,otp):
    
    def InvalidOtp(msg_head,msg_body):
        if msg_head == "OTP Expired":
            return render_template("reset_password.html",opt_err_msg=msg_body , opt_expired=1 , enter_otp = 1)
        if msg_head == "OTP Invalid":
            return render_template("reset_password.html",opt_err_msg=msg_body , opt_invalid=1 , enter_otp = 1)
    ...
    if url=="login.html":
        if role == "student":
            ...
            saved_otp = data["otp"]
            otp_used = data["otp_used"]
            if otp_used == 1:
                msg_head = "OTP Expired"
                msg_body = "Security Code Expired, Please reset your password again" 
                return InvalidOtp(msg_head,msg_body)

            time_diff = datetime.datetime.now() - datetime.datetime.strptime(expiry_at,'%Y-%m-%d %H:%M:%S.%f')
            time_diff = round(time_diff.total_seconds()/3600)
            if  time_diff < 1 and check_password(str(otp),saved_otp):
                cursor.execute("UPDATE student SET S_pass = %s , first_login = %s, otp_used = %s WHERE S_email = %s",[pwd,1,1,email])
            else:
                msg_head = "OTP Expired" if int(time_diff) > 1 else "OTP Invalid"
                msg_body = "Security Code Expired, Please reset your password again" if time_diff > 1 else "Entered Code is not valid"
                return InvalidOtp(msg_head,msg_body)

        elif role == "faculty":
            cursor.execute("SELECT otp,expiry_at,otp_used from faculty where F_email = %s",[email])
            data = cursor.fetchone()
            expiry_at = data["expiry_at"]
            saved_otp = data["otp"]
            otp_used = data["otp_used"]
            ...
            time_diff = round(time_diff.total_seconds()/3600)
            if  time_diff < 1 and check_password(str(otp),saved_otp):
                cursor.execute("UPDATE faculty SET F_pass = %s , first_login = %s , otp_used = %s WHERE F_email = %s",[pwd,1,1,email])
            else:
                msg_head = "OTP Expired" if int(time_diff) > 1 else "OTP Invalid"
                msg_body = "Security Code Expired, Please reset your password again" if time_diff > 1 else "Entered Code is not valid"
                return InvalidOtp(msg_head,msg_body)
            ...
            cursor.execute("SELECT otp,expiry_at,otp_used from admin where A_email = %s",[email])
            data = cursor.fetchone()
            expiry_at = data["expiry_at"]
            saved_otp = data["otp"]
            otp_used = data["otp_used"]
            ...
            time_diff = round(time_diff.total_seconds()/3600)
            if  time_diff < 1 and check_password(str(otp),saved_otp):
                cursor.execute("UPDATE admin SET A_pass = %s , first_login = %s, otp_used = %s WHERE A_email = %s",[pwd,1,1,email])
            else:
                msg_head = "OTP Expired" if int(time_diff) > 1 else "OTP Invalid"
                msg_body = "Security Code Expired, Please reset your password again" if time_diff > 1 else "Entered Code is not valid"
                return InvalidOtp(msg_head,msg_body)
...
def send_verification_mail(email,pwd,role,called_from):
    ...
    if called_from == "Login" or called_from=="Pass_Change":
        ...
        if role == 'student':
            cursor.execute("UPDATE student SET otp = %s,expiry_at = %s , otp_used = %s WHERE S_email = %s",(hashed_otp,time_now,0,email))
        if role == 'faculty':
            cursor.execute("UPDATE faculty SET otp = %s,expiry_at = %s , otp_used = %s WHERE F_email = %s",(hashed_otp,time_now,0,email))
        if role == 'admin':
            cursor.execute("UPDATE admin SET otp = %s,expiry_at = %s , otp_used = %s WHERE A_email = %s",(hashed_otp,time_now,0,email))
        
        mysql.connection.commit()
        cursor.close()
        msg.html = f"<p>Use this Security Code to reset your password. Code valid for one hour</p><br /> <h2>{otp}</h2>"
        mail.send(msg)
        print("MAIL SENT")
        if called_from == "Login":
            # session["flash_msg"] = "OTP sent if the email address provided is valid..."
            return redirect(url_for("resetpassword"))
 
        elif called_from == "Pass_Change":
            # f_msg = {}
            # f_msg['title'] = "OTP sent if the email address provided is valid..." 
            # f_msg['there'] = 1
            # f_msg['mode'] = 1
            # session['message'] = f_msg
            return redirect(url_for("resetpassword"))
...
def resetpassword():  
    if request.method == "GET":
        try:
            ...
            try:
                assert session["reset_email"]
            except KeyError:
                return redirect(url_for("show_login"))
            return render_template("reset_password.html",enter_otp="true")
    if request.method == "POST":
        try:
            ...
            return auth_decode(url = "login.html",pwd = pwd,otp=otp)
...
def dashboard():
    ...
    if mode=="Faculty":
        ...
        batch = session['batch']
        svv = session["svv"]
        cursor.execute('SELECT s.*,qd.q_title FROM score s,quiz_det qd WHERE s.quiz_id = qd.quiz_id;')        
